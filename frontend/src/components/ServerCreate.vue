          ...
          <v-card-text>
            <v-form v-model="valid">
              <v-layout justify-space-between wrap>
                <v-flex xs12>
                  <v-text-field v-model="serverName" label="Name" :disabled="$store.state.query.loading" :rules="[rules.required]"></v-text-field>
                </v-flex>
                <v-flex xs12 md4>
                  <v-text-field v-model="serverPort" label="Port" type="number" :disabled="$store.state.query.loading" :rules="[rules.required]"></v-text-field>
                </v-flex>
                <v-flex xs12 md4>
...
export default {
  ...
  methods: {
    ...
    getAvailablePort() {
      return Math.max(...this.servers.map(server => server.virtualserver_port)) + 1
    },
    async createServer() {
      try {
