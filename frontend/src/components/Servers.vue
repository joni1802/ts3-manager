  ...
  <v-container>
    <v-layout>
      <v-flex md8 sm10 xs12 offset-md2 offset-sm1>
        <v-card>
          <v-card-text>
              <v-data-table :headers="headers" :items="servers" item-key="virtualserver_id" :rows-per-page-items="rowsPerPage">
...
<script>
  ...
  export default {
    data() {
      return {
        headers: [
          {text: 'Selection', align: 'left', value: 'selection', sortable: false},
          {text: 'Name', align: 'left', value: 'virtualserver_name', sortable: false},
          {text: 'Port', align: 'left', value: 'virtualserver_port', sortable: false},
          {text: 'Clients', align: 'left', value: 'virtualserver_clientsonline_maxclients', sortable: false},
          {text: 'Uptime (d:h:m:s)', align: 'left', value: 'virtualserver_uptime', sortable: false},
          {text: 'Status', align: 'left', value: 'virtualserver_status', sortable: false},
        ],
        serverId: null,
        servers: [],
        stopDialog: false,
    ...
    methods: {
      ...
      async startServer() {
        try {
          ...
          this.$toast.error(err.msg, {icon: 'error'})
        }
