    ...
    <v-layout>
      <v-flex md8 sm10 xs12 offset-md2 offset-sm1>
        <v-card>
          ...
          <v-card-text>
            <v-data-table :headers="headers" :items="preparedBanlist" v-model="selected" item-key="banid" select-all :rows-per-page-items="rowsPerPage" :search="filter">
...
<script>
  export default {
    data() {
      return {
        headers: [
          {text: 'Name/IP/UID', value: 'name_ip_uid'},
          {text: 'Reason', value: 'reason'},
          {text: 'Expires', value: 'duration'}
        ],
        banlist: [],
        selected: [],
        dialog: false,
    ...
    computed: {
      ...
      preparedBanlist() {
        return this.banlist.map(ban => {
          return {...ban, 'name_ip_uid': `${ban.name} ${ban.uid} ${ban.ip}`}
        })
      }
    },
    methods: {
      ...
      addBan() {
        this.$router.push({path: '/ban/add'})
      },
      editBan(){
        this.$router.push({name: 'ban-edit', params:{banid: this.selected[0].banid}})
      },
      async deleteBans() {
        try {
