      ...
      <v-flex xs12>
        <permission-table :grantedPermissions="permissions" type="Channel Permissions" :editableContent="['permvalue']" @save="savePermission" @remove="removePermission" @loaded="init">
          <template slot="selectMenu">
            <v-flex sm3 xs12>
              <v-autocomplete :items="channelSelection" v-model="selectedChannel" @change="changeChannel" label="Channel"></v-autocomplete>
            </v-flex xs12>
          </template>
        </permission-table>
      </v-flex>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeChannel(cid) {
        this.$router.push({name: 'permissions-channel', params: {cid: cid}})
      },
      async init() {
        try {
